    ...
    <section id="about" style={{ padding: 'clamp(60px, 8vw, 120px) clamp(16px, 4vw, 28px)', background: '#fff' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'grid', gap: 64, alignItems: 'center' }} className="grid-cols-1 lg:grid-cols-2">
          ...
          <div>
            <p className="reveal" style={{ fontFamily: 'Instrument Sans', fontWeight: 700, fontSize: 21, letterSpacing: '0.14em', color: '#4CAF50', marginBottom: 14 }}>{a.tag}</p>
            ...
            <p className="reveal reveal-delay-2" style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#6B7280', lineHeight: 1.8, marginBottom: 14 }}>{a.p1}</p>
            <p className="reveal reveal-delay-2" style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', lineHeight: 1.8, marginBottom: 48 }}>{a.p2}</p>
            ...
            <div className="reveal reveal-delay-3 reveal reveal-delay-3 stats-grid"  style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 44 }}>
              {[
                ...
                <div key={i} style={{ background: '#FAFAFA', borderRadius: 20, padding: '22px 16px', textAlign: 'center', borderTop: `4px solid ${s.top}` }}>
                  ...
                  <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', marginTop: 5 }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="reveal reveal-delay-4" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 18, background: '#FAFAFA', borderRadius: 18, padding: '18px 22px', border: '1px solid rgba(13,63,107,0.06)', transition: 'all 0.3s cubic-bezier(0.16,1,0.3,1)', cursor: 'default' }}
                  ...
                  <div style={{ width: 48, height: 48, borderRadius: 16, background: v.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, flexShrink: 0 }}>{v.emoji}</div>
                  <div>
                    <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.02em' }}>{v.title}</p>
                    <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', marginTop: 2 }}>{v.desc}</p>
                  </div>
                </div>
              ))}
            </div>
