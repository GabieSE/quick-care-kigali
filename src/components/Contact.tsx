...
export default function Contact() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    fontFamily: 'Instrument Sans', fontSize: 21, color: '#0D3F6B',
    ...
    <section id="contact" style={{ padding: 'clamp(60px, 8vw, 120px) clamp(16px, 4vw, 28px)', background: '#FAFAFA' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div className="reveal" style={{ textAlign: 'center', marginBottom: 64 }}>
          <p style={{ fontFamily: 'Instrument Sans', fontWeight: 700, fontSize: 21, letterSpacing: '0.14em', color: '#4CAF50', marginBottom: 12 }}>{ct.tag}</p>
          <h2 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 'clamp(42px,5vw,64px)', letterSpacing: '-0.04em', color: '#0D3F6B', marginBottom: 16 }}>{ct.title}</h2>
          <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, maxWidth: 420, margin: '0 auto' }}>{ct.sub}</p>
        </div>
        ...
        <div className="grid-mobile-1" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48 }}>
          <div className="reveal" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {infoItems.map((item, i) => {
              ...
              const inner = (
                <div style={{ display: 'flex', alignItems: 'center', gap: 18, background: '#fff', borderRadius: 20, padding: '20px 24px', border: '1px solid rgba(13,63,107,0.07)', boxShadow: '0 4px 16px rgba(13,63,107,0.04)', transition: 'all 0.25s', cursor: item.href ? 'pointer' : 'default' }}
                  ...
                  <div>
                    <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', marginBottom: 2 }}>{item.label}</p>
                    <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.02em' }}>{item.value}</p>
                  </div>
                </div>
              )
              return item.href
            ...
            <div style={{ borderRadius: 20, overflow: 'hidden', height: 180, background: 'linear-gradient(135deg, #EEF6FF, #F0FFF4)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(13,63,107,0.07)', flexDirection: 'column', gap: 8 }}>
              ...
              <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF' }}>{ct.neighborhood}</p>
            </div>
          </div>
          ...
          <div className="reveal reveal-delay-2" style={{ background: '#fff', borderRadius: 28, padding: '40px 36px', border: '1px solid rgba(13,63,107,0.07)', boxShadow: '0 16px 48px rgba(13,63,107,0.08)' }}>
            <h3 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 26, color: '#0D3F6B', letterSpacing: '-0.03em', marginBottom: 28 }}>{ct.formTitle}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <textarea name="message" placeholder={ct.message} value={form.message} onChange={handle} rows={4}
                ...
                onFocus={e => e.target.style.borderColor = '#4CAF50'} onBlur={e => e.target.style.borderColor = 'rgba(13,63,107,0.12)'} />
              <button onClick={submit} style={{ background: '#0D3F6B', color: '#fff', padding: '16px', borderRadius: 999, border: 'none', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, letterSpacing: '-0.01em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: '0 8px 28px rgba(13,63,107,0.3)', transition: 'all 0.25s' }}
