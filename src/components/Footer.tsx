      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(40px, 6vw, 80px) clamp(16px, 4vw, 28px) 40px' }}>
        <div className="footer-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1.5fr', gap: 48, marginBottom: 64 }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: 'Instrument Sans', color: 'rgba(255,255,255,0.45)', fontSize: 21, lineHeight: 1.8, marginBottom: 28, maxWidth: 260 }}>{f.tagline}</p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {navLinks.map(([label, id]) => (
                <button key={id} onClick={() => go(id)} style={{ background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0, fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.45)', transition: 'color 0.2s' }}
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {['🧹 Home Cleaning', '👨‍🍳 Cooking Help', '🛒 Shopping', '👶 Baby Care', '🚨 Emergency'].map(s => (
                <span key={s} style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.45)' }}>{s}</span>
              ))}
            </div>
          </div>
              ...
              {[
                ...
                const inner = (
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <span style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.45)', wordBreak: 'break-all' }}>{c.val}</span>
                  </div>
                )
                return c.href
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 32, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.25)' }}>{f.rights}</p>
          <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.25)' }}>{f.madeIn}</p>
        </div>
      </div>
    </footer>
  )
