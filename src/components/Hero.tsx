        ...
        <div style={{ display: 'grid', gap: 56, alignItems: 'center' }} className="grid-cols-1 lg:grid-cols-2">
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.10)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: 999, padding: '8px 20px', marginBottom: 32 }}>
              ...
              <span style={{ fontFamily: 'Instrument Sans', fontWeight: 600, fontSize: 21, color: 'rgba(255,255,255,0.88)' }}>{t.hero.badge}</span>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#4CAF50', boxShadow: '0 0 8px #4CAF50' }} />
            </div>

            <h1 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 'clamp(50px, 6.5vw, 88px)', lineHeight: 1.0, letterSpacing: '-0.04em', color: '#fff', marginBottom: 28 }}>
              {t.hero.headline1}<br />
            ...
            <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, lineHeight: 1.7, color: 'rgba(255,255,255,0.65)', maxWidth: 460, marginBottom: 44 }}>{t.hero.sub}</p>

            <div className="hero-ctas" style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 52 }}>
              <a href="https://wa.me/250795628517" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 9, background: '#4CAF50', color: '#fff', padding: '17px 36px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, letterSpacing: '-0.01em', textDecoration: 'none', boxShadow: '0 8px 36px rgba(76,175,80,0.45)', transition: 'all 0.3s cubic-bezier(0.16,1,0.3,1)' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.05)' }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)' }}>
                <MessageCircle size={20} /> {t.hero.cta1}
              </a>
              <button onClick={() => go('services')} style={{ display: 'flex', alignItems: 'center', gap: 9, background: 'rgba(255,255,255,0.10)', color: '#fff', border: '1.5px solid rgba(255,255,255,0.25)', padding: '17px 32px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, letterSpacing: '-0.01em', cursor: 'pointer', transition: 'all 0.3s' }}
              ...
              {[{ icon: Shield, label: t.hero.badge1, color: '#4CAF50' }, { icon: Zap, label: t.hero.badge2, color: '#F97316' }, { icon: Star, label: t.hero.badge3, color: '#EAB308' }].map((b, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 999, padding: '9px 18px' }}>
                    ...
                    <span style={{ fontFamily: 'Instrument Sans', fontWeight: 600, fontSize: 21, color: 'rgba(255,255,255,0.82)' }}>{b.label}</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="hidden lg:block" style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', bottom: -20, left: -28, background: '#fff', borderRadius: 22, padding: '16px 22px', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', display: 'flex', alignItems: 'center', gap: 14, animation: 'floatY 6s ease-in-out infinite' }}>
              ...
              <div>
                <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 26, color: '#0D3F6B', lineHeight: 1, letterSpacing: '-0.03em' }}>500+</p>
                <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', marginTop: 3 }}>{t.about.stat1Label}</p>
              </div>
            </div>
            <div style={{ position: 'absolute', top: -16, right: -20, background: '#fff', borderRadius: 22, padding: '14px 20px', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', display: 'flex', alignItems: 'center', gap: 12, animation: 'floatY 8s ease-in-out infinite 1.5s' }}>
              ...
              <div>
                <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 24, color: '#0D3F6B', lineHeight: 1, letterSpacing: '-0.03em' }}>4.9 / 5</p>
                <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', marginTop: 3 }}>{t.about.stat2Label}</p>
              </div>
            </div>
          </div>
        </div>
