    ...
    <section id="how-it-works" style={{ padding: 'clamp(60px, 8vw, 120px) clamp(16px, 4vw, 28px)', background: '#fff' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div className="reveal" style={{ textAlign: 'center', marginBottom: 72 }}>
          <p style={{ fontFamily: 'Instrument Sans', fontWeight: 700, fontSize: 21, letterSpacing: '0.14em', color: '#4CAF50', marginBottom: 12 }}>{h.tag}</p>
          <h2 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 'clamp(42px,5vw,64px)', letterSpacing: '-0.04em', color: '#0D3F6B', marginBottom: 16 }}>{h.title}</h2>
          <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, maxWidth: 400, margin: '0 auto' }}>{h.sub}</p>
        </div>
        ...
        <div className="grid-mobile-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32, position: 'relative' }}>
          ...
          {steps.map((step, i) => (
            <div key={i} className="reveal" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative', zIndex: 1, transitionDelay: `${i * 0.15}s` }}>
              ...
              <h3 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 24, color: '#0D3F6B', letterSpacing: '-0.02em', marginBottom: 10 }}>{step.title}</h3>
              <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, lineHeight: 1.75, maxWidth: 260 }}>{step.desc}</p>
            </div>
          ))}
        </div>
