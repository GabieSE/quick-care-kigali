    ...
    <section id="services" style={{ padding: 'clamp(60px, 8vw, 120px) clamp(16px, 4vw, 28px)', background: '#FAFAFA' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div className="reveal" style={{ textAlign: 'center', marginBottom: 72 }}>
          <p style={{ fontFamily: 'Instrument Sans', fontWeight: 700, fontSize: 21, letterSpacing: '0.14em', color: '#4CAF50', marginBottom: 14 }}>
            {t.services.tag}
          </p>
          <h2 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 'clamp(42px,5vw,68px)', letterSpacing: '-0.04em', color: '#0D3F6B', marginBottom: 10 }}>
            {t.services.title}
          </h2>
          {/* Emergency Support temporarily - plain text matching Lovable */}
          <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#6B7280', marginBottom: 16 }}>
            Emergency Support temporarily
          </p>
          <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, maxWidth: 500, margin: '0 auto' }}>
            {t.services.sub}
          </p>
        </div>
          ...
          {servicesData.map((sv, i) => (
            <div key={sv.id} className="reveal" onClick={() => openModal(i)}
              ...
              <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
                <img src={sv.image} alt={sv.title}
                  style={{
                    ...
                    objectFit: 'cover',
                    objectPosition: sv.id === 'shopping' ? 'top center' : 'center 20%',
                  }}
              ...
              <div style={{ padding: '26px 28px 28px' }}>
                <h3 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 26, color: '#0D3F6B', letterSpacing: '-0.03em', marginBottom: 10 }}>{sv.title}</h3>
                <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, lineHeight: 1.7, marginBottom: 22 }}>{sv.tagline}</p>
                <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
                  <a href={`https://wa.me/250795628517?text=${encodeURIComponent(`Hi! I'd like to book: ${sv.title}`)}`}
                    target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                    style={{ flex: 1, background: '#25D366', color: '#fff', padding: '12px 8px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, textAlign: 'center', textDecoration: 'none', letterSpacing: '-0.01em', transition: 'opacity 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.opacity = '0.85'}
                    onMouseLeave={e => e.currentTarget.style.opacity = '1'}>
                    💬 WhatsApp
                  </a>
                  <a href="tel:+250795628517" onClick={e => e.stopPropagation()}
                    style={{ flex: 1, background: '#0D3F6B', color: '#fff', padding: '12px 8px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, textAlign: 'center', textDecoration: 'none', letterSpacing: '-0.01em', transition: 'opacity 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.opacity = '0.85'}
                    onMouseLeave={e => e.currentTarget.style.opacity = '1'}>
                    📞 Call Now
                  </a>
                </div>
                <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: sv.accentColor, fontWeight: 600, textAlign: 'center' }}>
        ...
        <div onClick={closeModal} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ position: 'relative', height: 200, overflow: 'hidden', borderRadius: '28px 28px 0 0', flexShrink: 0 }}>
              <img src={s.image} alt={s.title} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: s.id === 'shopping' ? 'top center' : 'center 20%' }} />
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.72) 0%, transparent 50%)' }} />
              <button onClick={closeModal} style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(8px)', border: 'none', borderRadius: 999, width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#0D3F6B', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', transition: 'transform 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.1)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
                <X size={18} />
              </button>
              <div style={{ position: 'absolute', bottom: 18, left: 22 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  <span style={{ fontSize: 26 }}>{s.emoji}</span>
                  <h3 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 26, color: '#fff', letterSpacing: '-0.03em' }}>{s.title}</h3>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: '24px 24px 28px' }}>
              ...
              {/* Description */}
              <p style={{ fontFamily: 'Instrument Sans', color: '#6B7280', fontSize: 21, lineHeight: 1.8, marginBottom: 20, fontStyle: 'italic' }}>
                {s.description}
              </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18, marginBottom: 24 }}>
                {s.sections.map((section, si) => (
                  <div key={si}>
                    <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.01em', marginBottom: 10 }}>
                      {section.heading}
                    </p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {section.items.map((item, ii) => (
                        <div key={ii} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          ...
                          <span style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#4B5563', lineHeight: 1.6 }}>{item}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{ background: '#F8FAFB', borderRadius: 18, padding: '18px 20px', marginBottom: 22 }}>
                <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.02em', marginBottom: 14 }}>
                  💰 Service Price
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {s.pricing.map((p, pi) => (
                    <div key={pi} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                      <span style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#6B7280' }}>{p.label}</span>
                      <span style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, color: s.accentColor, whiteSpace: 'nowrap' }}>{p.price}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: booked ? '#F0FFF4' : 'linear-gradient(135deg, #071F35, #0D3F6B)', borderRadius: 20, padding: '20px', marginBottom: 4, transition: 'all 0.4s ease' }}>
                {!booked ? (
                  <>
                    <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 21, color: '#fff', letterSpacing: '-0.02em', marginBottom: 4 }}>
                      📋 How to Book
                    </p>
                    <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: 'rgba(255,255,255,0.65)', marginBottom: 16, lineHeight: 1.6 }}>
                    ...
                    <div className="booking-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      {bookingChannels.map((ch, i) => (
                        <a key={i} href={ch.href(s.title)}
                          ...
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: ch.color, color: '#fff', padding: '13px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, textDecoration: 'none', letterSpacing: '-0.01em', boxShadow: `0 4px 16px ${ch.color}55`, transition: 'all 0.25s' }}
                  ...
                  <div style={{ textAlign: 'center', padding: '8px 0' }}>
                    ...
                    <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.02em', marginBottom: 4 }}>Booking Started!</p>
                    <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#6B7280' }}>We'll confirm your booking shortly. Thank you!</p>
                  </div>
                )}
              </div>
              ...
              <button onClick={closeModal} style={{
                ...
                background: '#F8FAFB', border: '1.5px solid rgba(13,63,107,0.12)',
                borderRadius: 16, cursor: 'pointer', display: 'flex', alignItems: 'center',
                justifyContent: 'center', gap: 8,
                fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21,
                color: '#6B7280', transition: 'all 0.2s',
              }}
                onMouseEnter={e => { e.currentTarget.style.background = '#EEF6FF'; e.currentTarget.style.color = '#0D3F6B' }}
                onMouseLeave={e => { e.currentTarget.style.background = '#F8FAFB'; e.currentTarget.style.color = '#6B7280' }}>
                ✕ Close
              </button>
            </div>
          </div>
