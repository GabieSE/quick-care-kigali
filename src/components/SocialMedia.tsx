    ...
    <section id="social" style={{ padding: 'clamp(60px, 8vw, 100px) clamp(16px, 4vw, 28px)', background: '#fff' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div className="reveal" style={{ textAlign: 'center', marginBottom: 64 }}>
          <p style={{ fontFamily: 'Instrument Sans', fontWeight: 700, fontSize: 21, letterSpacing: '0.14em', color: '#4CAF50', marginBottom: 12 }}>{s.tag}</p>
          <h2 style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 'clamp(42px,5vw,64px)', letterSpacing: '-0.04em', color: '#0D3F6B', marginBottom: 16 }}>{s.title}</h2>
          <p style={{ fontFamily: 'Instrument Sans', color: '#9CA3AF', fontSize: 21, maxWidth: 420, margin: '0 auto' }}>{s.sub}</p>
        </div>
          ...
          {socials.map((sv, i) => {
            ...
            return (
              <a key={i} href={sv.link} target="_blank" rel="noopener noreferrer" className="reveal"
                ...
                <p style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 21, color: '#0D3F6B', letterSpacing: '-0.02em', marginBottom: 4 }}>{sv.name}</p>
                <p style={{ fontFamily: 'Instrument Sans', fontWeight: 600, fontSize: 21, color: '#4CAF50', marginBottom: 8 }}>{sv.handle}</p>
                <p style={{ fontFamily: 'Instrument Sans', fontSize: 21, color: '#9CA3AF', lineHeight: 1.6, marginBottom: 20 }}>{sv.desc}</p>
                <span style={{ background: sv.bg, color: '#fff', padding: '8px 22px', borderRadius: 999, fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 21, letterSpacing: '-0.01em' }}>
                  {sv.emoji} {s.follow}
                </span>
              </a>
            )
